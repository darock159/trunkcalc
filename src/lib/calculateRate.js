...
function selectSet(tree, inj) {
    if (inj === "Tree IV")
        return { source: "TREE IV", data: treeiv };
    if (inj === "Arbotect")
        return { source: "ARBOTECT", data: arbotect[tree] || [] };
    if (inj === "Oak Wilt")
        return { source: "ALAMO", data: oakWilt["Oak - Wilt"] || [] };
    return { source: "IRON", data: iron[tree] || [] };
}
export function calculateRate(dbh, treeType, injectionType) {
    const rounded = Math.round(dbh);
    const tree = treeType;
    const inj = injectionType;
    ...
    const rows = (Array.isArray(data) ? data : [])
        ...
        .filter((r) => Number.isFinite(r._dbh))
        .sort((a, b) => a._dbh - b._dbh);
    if (inj === "Arbotect" && tree === "Elm" && rounded < 10) {
        return { dbh: rounded, notRecommended: true, source };
    }
    const exact = rows.find((r) => r._dbh === rounded);
    if (exact)
        return shape(source, rounded, exact);
    if (rows.length) {
        const max = rows[rows.length - 1]._dbh;
        if (rounded <= max) {
            const lower = rows.filter((r) => r._dbh <= rounded).pop();
            if (lower)
                return shape(source, rounded, lower);
        }
        // overflow behaviors
        if (rounded > max) {
    ...
    if (inj === "Arbotect" && rounded === 50) {
        if (tree === "Elm")
            return { dbh: 50, product: 120, water: 60, units: { product: "oz", water: "gal" }, source };
        if (tree === "Sycamore")
            return { dbh: 50, product: 120, water: 30, units: { product: "oz", water: "gal" }, source };
    }
    return { dbh: rounded, product: 0, units: "", source };
}
function shape(source, dbh, row) {
    ...
    if (source === "ALAMO" && (row.productLow != null || row.productHigh != null)) {
        const out = { dbh, units: { product: "oz", water: "gal" }, source };
        if (row.productLow != null)
            out.productLow = Number(row.productLow);
        if (row.productHigh != null)
            out.productHigh = Number(row.productHigh);
        if (Number.isFinite(waterNum))
            out.water = waterNum;
        return out;
    }
    if (source === "TREE IV") {
    ...
    const out = { dbh, product: Number(prod) || 0, units: { product: "oz", water: "gal" }, source };
    if (Number.isFinite(waterNum))
        out.water = waterNum;
    return out;
}
